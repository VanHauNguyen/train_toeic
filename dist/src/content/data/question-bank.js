...
exports.QUESTION_TOPICS = exports.TOEIC_PART5_QUESTION_BANK = void 0;
...
function addContext(question, index, baseCount = 5) {
    if (index < baseCount)
        return question;
    return question.replace(/\.$/, ` ${contextPhrases[index % contextPhrases.length]}.`);
}
function build(topic, seeds) {
    return seeds.map((seed, index) => ({
        id: `p5-${topic.replace(/\s+/g, '-')}-${String(index + 1).padStart(3, '0')}`,
        toeicPart: client_1.ToeicPart.PART5,
        topic,
        difficulty: seed.difficulty ?? client_1.SkillLevel.BEGINNER,
...
].map(([question, correct, distractors, vi, zhTW, tags]) => ({ question, correct, distractors, vi, zhTW, tags }));
const tense = Array.from({ length: 30 }, (_, index) => {
    const rows = [
...
const wordForms = Array.from({ length: 30 }, (_, index) => {
    const rows = [
...
function buildSeeds(topic, count, rows) {
...
exports.TOEIC_PART5_QUESTION_BANK = [
...
exports.QUESTION_TOPICS = [...new Set(exports.TOEIC_PART5_QUESTION_BANK.map((question) => question.topic))];
