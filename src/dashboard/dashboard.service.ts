...
export class DashboardService {
  ...
  async me(userId: string) {
    ...
    const partStats = new Map<string, { total: number; correct: number }>();
    ...
    for (const attempt of attempts) {
      for (const answer of attempt.answers) {
        ...
        if (!answer.isCorrect) {
          for (const tag of answer.question.tags) missedTags.set(tag, (missedTags.get(tag) ?? 0) + 1);
        }
  ...
  async adminAnalytics() {
    ...
    const partStats = new Map<string, { total: number; correct: number }>();
    const missed = new Map<string, { questionId: string; prompt: string; toeicPart: string; missed: number; total: number }>();
    for (const attempt of attempts) {
      for (const answer of attempt.answers) {
    ...
    for (const question of difficultQuestions) {
      ...
      if (total > 0 && wrong > 0) missed.set(question.id, { questionId: question.id, prompt: question.prompt, toeicPart: question.toeicPart, missed: wrong, total });
    }
  ...
  private recommend(weakToeicParts: string[], correctRate: number, missedTags: string[]) {
    if (!weakToeicParts.length) return ['完成第一回模擬測驗，建立能力基準。'];
    const actions = weakToeicParts.map((part) => `加強 ${part} 題型，先複習錯題再做短練習。`);
    if (correctRate < 70) actions.push('每天安排 20 分鐘單字與文法基礎複習。');
    if (missedTags.length) actions.push(`針對常錯主題 ${missedTags.join('、')} 建立個人化練習。`);
    return actions;
  }
}
